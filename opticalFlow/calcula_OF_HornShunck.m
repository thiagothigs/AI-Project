function [uxv_HS] = calcula_OF_HornShunck()
...
% Retorna um objeto do Sistema de fluxo óptico, HOF, que estima a direção e velocidade do
% movimento do objeto de uma imagem para outra, ou de um quadro do vídeo para outro.
hof = vision.OpticalFlow('ReferenceFrameDelay', 1);
% Método de cálculo do Fluxo Óptico - Horn Shunck (Ver Bibliografia)
hof.Method = 'Horn-Schunck';
% hof.MaximumIterationCount = 50;
hof.Smoothness = 0.05;
% hof.IterationTerminationCondition = 'Maximum iteration count';

% Threshold for noise reduction
% Specify the motion threshold between each image or video frame as
% a positive scalar number. The higher the number, the less small 
% movements impact the optical flow calculation. 
% This property applies when you set the Method property to 
% Lucas-Kanade. This property is tunable. Default: 0.0039
...
% Toca o vídeo ou imagem de exibição: retorna um objeto do sistema de exibição de vídeo,
% HVP, para visualizar sequências de vídeo ou de imagem na tela no MATLAB.
...
while ~isDone(hvfr)%isDone: Indica se a operação está completa quando se trabalha com interface, ou seja, indica se o vídeo chegou ao final.
    ...
    %matrix where each element is of the form . If you select Horizontal and vertical
    %components in complex form, the block outputs the optical flow matrix where each
    %element is of the form u+j*v.
   
    n=n+1; % Número de quadros;
    uxv_HS{n} = of; % Matriz de fluxo óptico;
    %ite = ite + 1;
end
...
%release(hvp);%Libera a interface e todos os recursos utilizados pela interface;
%release(hvfr);%Libera a interface e todos os recursos utilizados pela interface;
